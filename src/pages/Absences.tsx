    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 py-6">
        ...
        <div className="bg-card/80 backdrop-blur-md border border-border rounded-2xl p-6 mb-6 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
          <h1 className="text-2xl font-bold text-foreground mb-1">Отсутствия</h1>
          <h2 className="text-lg font-semibold text-foreground mb-4">Добавить отсутствие</h2>
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-muted-foreground">Сотрудник:</label>
              <Select value={selectedEmployee} onValueChange={setSelectedEmployee}>
                <SelectTrigger className="w-72 bg-background/70">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {employees.map(emp => (
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="w-40 bg-background/70" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-muted-foreground">По:</label>
              <Input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="w-40 bg-background/70" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-medium text-muted-foreground">Причина:</label>
              <Input value={reason} onChange={e => setReason(e.target.value)} placeholder="Причина" className="w-48 bg-background/70" />
            </div>
